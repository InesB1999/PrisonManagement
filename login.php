<?php
  session_start();
  $db = mysqli_connect ("localhost", "root", "", "authenticate"); // to connect to the database 'authenticate', no password.

  if(isset($_POST["submit"])) {
    $username = ($_POST["username"]);
    $pass = ($_POST["pass"]);
    $pass = md5($pass); //for security reasons; hashes the password.
...
<!-- Creates the form where the users can input data -->
    ...
    <h4 class="text-center"> If you do not have an account, please register first by clicking on the register button </h4><br>
      ...
      <input type="password" placeholder="Enter Password" name="pass"  class="pass" required>
